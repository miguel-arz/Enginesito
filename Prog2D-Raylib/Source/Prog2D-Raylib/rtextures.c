...
//uvs (u1, v1, u2, v2)
//dest
//Origin (pivot X, Pivot Y
void DrawTextureProUV(Texture2D texture, Vector4 uvs, Rectangle dest, Vector2 origin, float rotation, Color tint)
{
    Rectangle source;
    source.x = texture.width * uvs.x; //pix inicio del Quad
    source.y = texture.height * uvs.y;
    source.width = (uvs.z-uvs.x) * texture.width;
    source.height = (uvs.w-uvs.y) * texture.height;

    origin.x = origin.x * dest.width;
    origin.y = origin.y * dest.height;
